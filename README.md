...
The thread split will be to have dedicated main and render thread (maybe more later if it becomes pertinent, i.e. profiling reveals something like audio or physics is taking an unproportionate amount of compute they may get a dedicated thread). Any thread that is in a waiting state will be free to take from the to do work of other threads, this will be done through a dequeue structure to reduce instances of two threads wanting to start the same job. There will be care taken for a maximum number of workers for background tasks so that there will be no latency hitches if no worker is free to handle some foreground task. As seen on the diagram dependencies will be managed through a central task graph. Tasks will have priorities and that will affect which type of task a thread will pick from a local set of dequeues when finishing a job. There will be scoping of tasks, which will allow for waiting on the completion of different scopes like physics or a frame before starting tasks of some other scope. This will also allow for future support of pausing the game whilst having the editor running.

Current plan of action:
 - Finish the task system (almost there just refining some internals)
